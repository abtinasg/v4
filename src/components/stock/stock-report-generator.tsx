...
export function StockReportGenerator({ symbol, companyName }: StockReportGeneratorProps) {
  ...
  const [viewerConfig, setViewerConfig] = useState<{ show: boolean; audienceType: AudienceType }>({ show: false, audienceType: 'pro' });
      ...
      <div className="relative p-6 lg:p-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
          ...
          <div className="p-4 rounded-xl bg-white/[0.02] border border-white/[0.06] hover:border-violet-500/30 transition-all group">
            ...
            <Button
              onClick={() => setViewerConfig({ show: true, audienceType: 'pro' })}
              size="sm"
              className="w-full h-10 bg-gradient-to-r from-violet-600 to-violet-500 hover:from-violet-500 hover:to-violet-400 text-white border-0 font-semibold text-xs transition-all duration-300 shadow-lg shadow-violet-500/20 rounded-lg group-hover:shadow-violet-500/30"
            >
          ...
          <div className="p-4 rounded-xl bg-white/[0.02] border border-white/[0.06] hover:border-cyan-500/30 transition-all group">
            ...
            <Button
              onClick={() => setViewerConfig({ show: true, audienceType: 'retail' })}
              size="sm"
              className="w-full h-10 bg-gradient-to-r from-cyan-600 to-cyan-500 hover:from-cyan-500 hover:to-cyan-400 text-white border-0 font-semibold text-xs transition-all duration-300 shadow-lg rounded-lg"
            >
              <Sparkles className="mr-2 h-3.5 w-3.5" />
      ...
      <AnimatePresence>
        {viewerConfig.show && (
          <AiPdfViewer 
            ...
            audienceType={viewerConfig.audienceType}
            onClose={() => setViewerConfig({ ...viewerConfig, show: false })}
          />
        )}
      </AnimatePresence>
    </motion.div>
