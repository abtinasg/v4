...
export default function StockAnalysisPage() {
  ...
  return (
    <div className="space-y-6 sm:space-y-8 p-3 sm:p-4 lg:p-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl sm:text-3xl font-bold text-white">Stock Analysis</h1>
        <p className="text-gray-400 mt-1 text-sm sm:text-base">Deep dive into individual stocks with 150+ institutional-grade metrics</p>
      </div>
        ...
        <CardContent className="pt-4 sm:pt-6">
          <form onSubmit={handleSearch} className="relative">
            ...
            <input
              ...
              className="w-full pl-10 sm:pl-12 pr-4 py-3 sm:py-4 bg-white/[0.03] border border-white/[0.08] rounded-xl text-sm sm:text-base text-white placeholder-gray-500 focus:outline-none focus:border-[#00D4FF]/50 focus:ring-1 focus:ring-[#00D4FF]/30 transition-all"
            />
            <button
              type="submit"
              className="absolute right-2 top-1/2 -translate-y-1/2 px-3 sm:px-5 py-1.5 sm:py-2 bg-gradient-to-r from-[#00D4FF] to-[#3B82F6] rounded-lg text-xs sm:text-sm font-medium text-[#05070B] hover:shadow-[0_0_20px_rgba(0,212,255,0.3)] transition-all"
            >
              Analyze
            </button>
          </form>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
          {popularStocks.map((stock) => (
            <button
              ...
              className="group p-4 rounded-xl border border-white/[0.05] bg-[#0A0D12]/40 hover:border-[#00D4FF]/30 hover:bg-[#00D4FF]/[0.03] transition-all duration-300 text-left"
            >
              <div className="flex items-center justify-between mb-2">
                <span className="text-lg font-semibold text-white">{stock.symbol}</span>
                <ChevronRight className="h-4 w-4 text-gray-600 group-hover:text-[#00D4FF] transition-colors" />
              </div>
              <p className="text-sm text-gray-500 truncate">{stock.name}</p>
              <span className="inline-block mt-2 text-xs px-2 py-1 rounded-full bg-white/[0.03] text-gray-400">
                {stock.sector}
              </span>
            </button>
          ))}
      ...
      <div>
        <div className="flex items-center gap-2 mb-4">
          <SlidersHorizontal className="h-5 w-5 text-[#22C55E]" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <button
            ...
            className="group p-6 rounded-xl border border-white/[0.05] bg-[#0A0D12]/40 hover:border-[#00D4FF]/30 hover:bg-[#00D4FF]/[0.03] transition-all duration-300 text-left"
          >
            <div className="flex items-center gap-4">
              <div className="p-3 rounded-lg bg-[#00D4FF]/10">
                <GitCompareArrows className="h-6 w-6 text-[#00D4FF]" />
              </div>
              <div className="flex-1">
                <div className="flex items-center justify-between">
                  <span className="text-lg font-semibold text-white">Compare Stocks</span>
                  <ChevronRight className="h-5 w-5 text-gray-600 group-hover:text-[#00D4FF] transition-colors" />
                </div>
                <p className="text-sm text-gray-500 mt-1">Compare up to 5 stocks side-by-side with visual charts</p>
              </div>
            </div>
          </button>
          <button
            onClick={() => router.push('/dashboard/stock-analysis/screener')}
            className="group p-6 rounded-xl border border-white/[0.05] bg-[#0A0D12]/40 hover:border-[#22C55E]/30 hover:bg-[#22C55E]/[0.03] transition-all duration-300 text-left"
          >
            <div className="flex items-center gap-4">
              <div className="p-3 rounded-lg bg-[#22C55E]/10">
                <SlidersHorizontal className="h-6 w-6 text-[#22C55E]" />
              </div>
              <div className="flex-1">
                <div className="flex items-center justify-between">
                  <span className="text-lg font-semibold text-white">Stock Screener</span>
                  <ChevronRight className="h-5 w-5 text-gray-600 group-hover:text-[#22C55E] transition-colors" />
                </div>
                <p className="text-sm text-gray-500 mt-1">Filter stocks by metrics, sectors, and quality scores</p>
              </div>
            </div>
          </button>
        </div>
      ...
      <div>
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="text-lg font-semibold text-white">Available Metrics</h2>
          <span className="text-sm text-gray-500">({metricCategories.reduce((acc, cat) => acc + cat.count, 0)}+ total)</span>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {metricCategories.map((category) => (
            <div
              key={category.name}
              className="p-4 rounded-xl border border-white/[0.05] bg-[#0A0D12]/40"
            >
              <div className="text-2xl font-semibold tabular-nums" style={{ color: category.color }}>
                {category.count}
              </div>
